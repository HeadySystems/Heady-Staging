...
const {
  ...
  CSL_THRESHOLDS, SERVICE_PORTS, COLAB_RUNTIMES,
...
const BRIDGE_CONFIG = {
  heartbeatIntervalMs: fib(8) * 1000,       // 21s heartbeat
  heartbeatTimeoutMs: fib(10) * 1000,        // 55s timeout
  maxConcurrentTasks: fib(9),                 // 34 concurrent tasks
  taskTimeoutMs: fib(13) * 1000,              // 233s task timeout
  resultBufferSize: fib(12),                  // 144 buffered results
  reconnectMaxAttempts: fib(7),               // 13 reconnect attempts
  maxMessageSize: fib(20) * 1024,             // ~6.9MB max message
...
class ColabBridge {
  constructor() {
    ...
    if (this.runtimes.size === 0) {
      const defaultRuntimes = {
        alpha: { name: 'Runtime Alpha', role: 'inference', gpu: 'A100', tunnelPort: 8501 },
        beta:  { name: 'Runtime Beta',  role: 'memory',    gpu: 'A100', tunnelPort: 8502 },
        gamma: { name: 'Runtime Gamma', role: 'training',  gpu: 'A100', tunnelPort: 8503 },
      };
  ...
  async start() {
    ...
    return new Promise((resolve) => {
      this.server.listen(PORT, () => {
        logger.info({ message: 'Colab Bridge started', port: PORT, runtimes: this.runtimes.size });
  ...
  async _handleTaskSubmission(req, res) {
    ...
    if (!runtime) {
      this._respondJson(res, 503, { error: 'No healthy runtime available', taskId });
      return;
    }
    ...
    this.taskCounter++;
    this._respondJson(res, 202, { taskId, status: taskRecord.status, runtime: runtime.id });
  }
  ...
  _handleTaskStatus(req, res, url) {
    ...
    for (const [, runtime] of this.runtimes) {
      ...
      if (task) {
        this._respondJson(res, 200, { taskId, status: task.status, runtime: runtime.id });
        return;
      }
    }
    ...
    if (result) {
      this._respondJson(res, 200, { taskId, status: result.status, hasResult: !!result.result });
      return;
    }

    this._respondJson(res, 404, { error: 'Task not found', taskId });
  }
  ...
  _handleTaskResult(req, res, url) {
    ...
    if (!result) {
      this._respondJson(res, 404, { error: 'Result not found', taskId });
      return;
    }
  ...
  _handleUpgrade(req, socket, head) {
    ...
    const acceptKey = crypto
      .createHash('sha1')
      .update(key + '258EAFA5-E914-47DA-95CA-5AB5DC587235')
      .digest('base64');
    ...
    logger.info({ message: 'Colab runtime connected', runtimeId, role: runtime.config.role });

    // Start heartbeat
    runtime.heartbeatTimer = setInterval(() => {
      if (runtime.connected) {
        try {
          this._sendWsFrame(socket, JSON.stringify({ type: 'ping', timestamp: Date.now() }));
        } catch {
    ...
    socket.on('error', (err) => {
      logger.error({ message: 'WebSocket error', runtimeId, error: err.message });
      this._handleRuntimeDisconnect(runtimeId);
    });
  ...
  _handleRuntimeMessage(runtimeId, messageStr) {
    try {
      ...
      switch (msg.type) {
        ...
        default:
          logger.debug({ message: 'Unknown runtime message type', type: msg.type, runtime: runtimeId });
      }
    } catch (error) {
      logger.error({ message: 'Failed to parse runtime message', runtime: runtimeId, error: error.message });
    }
  }
  ...
  _handleRuntimeDisconnect(runtimeId) {
    ...
    logger.warn({ message: 'Colab runtime disconnected', runtimeId, requeuedTasks: this.taskQueue.length });
  }
  ...
  async shutdown() {
    logger.info({ message: 'Colab Bridge shutting down' });

    for (const [, runtime] of this.runtimes) {
      if (runtime.heartbeatTimer) clearInterval(runtime.heartbeatTimer);
      if (runtime.ws) {
        try { runtime.ws.destroy(); } catch (e) {
    ...
    logger.info({ message: 'Colab Bridge shut down cleanly' });
  }
}
...
if (require.main === module) {
  ...
  bridge.start().catch(err => {
    logger.error({ message: 'Bridge startup failed', error: err.message });
    process.exit(1);
  });
