...
const DEFAULT_BEE_TIMEOUT_MS = PHI_TIMING.CYCLE;
...
class BeeCB {
  constructor(domain, opts = {}) {
    ...
    this.recoveryMs = opts.recoveryMs ?? PHI_TIMING.CYCLE;
    this._failures = 0;
...
class BeeFactoryV2 extends EventEmitter {
   ...
   * @param {number}   [opts.defaultTimeoutMs=PHI_TIMING.CYCLE]
   */
  constructor(opts = {}) {
    super();
   ...
   * @param {number}  [policy.timeoutMs=PHI_TIMING.CYCLE]
            ...
            await new Promise((resolve) => {
              const tick = () => {
                while (queue.length > 0 && active.size < maxConcurrentBees) {
                  ...
                  runBee(bee).then(tick).catch(err => logger.error('Unhandled promise rejection', { error: err.message }));
                }
