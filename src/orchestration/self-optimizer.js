...
const fs = require("fs");
let { PHI_INTERVALS } = {}; try { { PHI_INTERVALS } = require("./vector-pipeline"); } catch(e) {
...
try { optState = { ...optState, ...JSON.parse(fs.readFileSync(OPT_FILE, "utf-8")) }; } catch (e) {
  logger.error('Unexpected error', { error: e.message, stack: e.stack });
}
...
function save() { try { fs.writeFileSync(OPT_FILE, JSON.stringify(optState, null, 2)); } catch (e) {
  logger.error('Unexpected error', { error: e.message, stack: e.stack });
} }
...
function discoverSkills() {
    ...
    try { fs.writeFileSync(path.join(SKILLS_DIR, "manifest.json"), JSON.stringify(skillManifest, null, 2)); } catch (e) {
      logger.error('Unexpected error', { error: e.message, stack: e.stack });
    }
...
async function runSystemScan() {
    ...
    try {
        ...
        for (const f of files) {
            try { totalSize += fs.statSync(path.join(dataDir, f)).size; } catch (e) {
              logger.error('Unexpected error', { error: e.message, stack: e.stack });
            }
        }
