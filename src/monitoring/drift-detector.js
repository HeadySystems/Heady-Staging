...
let pg, redis, axios;
try { pg    = require('pg');       } catch { /* eslint-disable-line no-empty */ }
try { redis = require('ioredis');  } catch { /* eslint-disable-line no-empty */ }
try { axios = require('axios');    } catch { /* eslint-disable-line no-empty */ }
...
class DriftDetector extends EventEmitter {
  ...
  async triggerRecalibration(opts = {}) {
    ...
    try {
      ...
      await client.query('ROLLBACK').catch(err => console.error('[drift-detector] rollback failed:', err.message || err));
    ...
    if (this._redisClient) {
      ...
      if (cached) {
        try {
          ...
          return;
        } catch (err) { console.error('[drift-detector] baseline cache parse failed:', err.message || err); }
      }
    }
