/**
 * heady-saga — Service Entry Point
 * Saga orchestrator — distributed txns, compensating rollback
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headysystems.com | Port: 3322
...
health.register('heady-event-store', async () => {
  const url = process.env['HEADY_EVENT_STORE_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_EVENT_STORE_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-event-store HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-cqrs', async () => {
  const url = process.env['HEADY_CQRS_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_CQRS_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-cqrs HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
