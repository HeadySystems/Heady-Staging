 ...
 * Auth — JWT Ed25519, mTLS, service identity, zero-trust
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headysystems.com | Port: 3309
...
health.register('heady-receipt-signer', async () => {
  const url = process.env['HEADY_RECEIPT_SIGNER_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_RECEIPT_SIGNER_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-receipt-signer HTTP ${r.status}` };
