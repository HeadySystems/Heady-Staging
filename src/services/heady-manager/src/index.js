/**
 * heady-manager — Service Entry Point
 * Primary orchestration manager — 21-stage pipeline controller
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headysystems.com | Port: 3301
...
health.register('heady-gateway', async () => {
  const url = process.env['HEADY_GATEWAY_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_GATEWAY_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-gateway HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-mcp', async () => {
  const url = process.env['HEADY_MCP_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_MCP_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-mcp HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-brain', async () => {
  const url = process.env['HEADY_BRAIN_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_BRAIN_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-brain HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
