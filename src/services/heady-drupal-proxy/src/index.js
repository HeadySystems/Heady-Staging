/**
 * heady-drupal-proxy — Service Entry Point
 * Drupal proxy — headyconnection.org content delivery
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headyconnection.org | Port: 3342
...
const SERVICE_NAME = 'heady-drupal-proxy';
...
health.register('heady-drupal', async () => {
  const url = process.env['HEADY_DRUPAL_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_DRUPAL_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-drupal HTTP ${r.status}` };
