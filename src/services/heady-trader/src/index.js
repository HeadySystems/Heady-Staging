/**
 * heady-trader — Service Entry Point
 * Heady Trader — autonomous trading, phi-scaled risk gates
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headyme.com | Port: 3347
...
const SERVICE_NAME = 'heady-trader';
...
health.register('heady-mc', async () => {
  ...
  if (!url) return { status: 'degraded', message: 'HEADY_MC_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-mc HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
