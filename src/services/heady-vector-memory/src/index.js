/**
 * heady-vector-memory — Service Entry Point
 * 3D vector memory — pgvector RAG, Graph traversal
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headysystems.com | Port: 3311
...
const SERVICE_NAME = 'heady-vector-memory';
...
health.register('heady-embeddings', async () => {
  const url = process.env['HEADY_EMBEDDINGS_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_EMBEDDINGS_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-embeddings HTTP ${r.status}` };
