/**
 * heady-auto-tuner — Service Entry Point
 * Auto-tuner — runtime phi-scaling, concurrency/timeout
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headysystems.com | Port: 3327
...
const SERVICE_NAME = 'heady-auto-tuner';
...
health.register('heady-observability', async () => {
  const url = process.env['HEADY_OBSERVABILITY_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_OBSERVABILITY_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-observability HTTP ${r.status}` };
