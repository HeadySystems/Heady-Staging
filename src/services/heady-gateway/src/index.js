/**
 * heady-gateway — Service Entry Point
 * API Gateway — CSL routing, auth, rate limiting
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headyapi.com | Port: 3302
...
const SERVICE_NAME = 'heady-gateway';
...
health.register('heady-auth', async () => {
  const url = process.env['HEADY_AUTH_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_AUTH_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-auth HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-router', async () => {
  const url = process.env['HEADY_ROUTER_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_ROUTER_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-router HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
