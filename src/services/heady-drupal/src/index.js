/**
 * heady-drupal — Service Entry Point
 * Drupal integration — headyconnection.org CMS bridge
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headyconnection.org | Port: 3310
...
const SERVICE_NAME = 'heady-drupal';
...
health.register('heady-cache', async () => {
  const url = process.env['HEADY_CACHE_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_CACHE_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-cache HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
