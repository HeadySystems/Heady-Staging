/**
 * heady-seventeen-swarm — Service Entry Point
 * 17-swarm orchestrator — golden-angle ring, CSL gates
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headybee.co | Port: 3331
...
const SERVICE_NAME = 'heady-seventeen-swarm';
...
health.register('heady-swarm-coordinator', async () => {
  const url = process.env['HEADY_SWARM_COORDINATOR_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_SWARM_COORDINATOR_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-swarm-coordinator HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-bee-factory', async () => {
  const url = process.env['HEADY_BEE_FACTORY_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_BEE_FACTORY_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-bee-factory HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
