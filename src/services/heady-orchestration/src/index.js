/**
 * heady-orchestration — Service Entry Point
 * Orchestration — task DAG, stage transitions, CSL gates
 * © 2026 HeadySystems Inc. — 60+ Provisional Patents
 *
 * Domain: headysystems.com | Port: 3307
...
health.register('heady-manager', async () => {
  ...
  if (!url) return { status: 'degraded', message: 'HEADY_MANAGER_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-manager HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-hive', async () => {
  const url = process.env['HEADY_HIVE_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_HIVE_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-hive HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});

health.register('heady-pipeline-core', async () => {
  const url = process.env['HEADY_PIPELINE_CORE_URL'];
  if (!url) return { status: 'degraded', message: 'HEADY_PIPELINE_CORE_URL not set' };
  try {
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(url + '/health/live', { signal: ctl.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-pipeline-core HTTP ${r.status}` };
  } catch (err) { return { status: 'unhealthy', message: err.message }; }
});
