...
const MODELS = {
    'gpt-5-nano': { costPerMInputTokens: 0.05, costPerMOutputTokens: 0.40, maxTokens: 8192 },
    'gpt-4.1-mini': { costPerMInputTokens: 0.40, costPerMOutputTokens: 1.60, maxTokens: 16384 },
    'gpt-4.1': { costPerMInputTokens: 2.00, costPerMOutputTokens: 8.00, maxTokens: 32768 },
};
...
class AzureOpenAIClient {
    constructor(endpoint = AZURE_ENDPOINT, apiKey = AZURE_API_KEY, apiVersion = AZURE_API_VERSION) {
        ...
        this._stats = { requests: 0, errors: 0, tokensIn: 0, tokensOut: 0, costUsd: 0 };
    }

    /**
     * Chat completion via Azure OpenAI.
     *
     * @param {string} deploymentName - Azure deployment name (e.g., 'gpt-5-nano')
     * @param {Array<{role: string, content: string}>} messages - Chat messages
     * @param {object} [opts] - Options (temperature, max_tokens, etc.)
     * @returns {Promise<object>} - Azure OpenAI response
     */
    async chatCompletion(deploymentName, messages, opts = {}) {
        ...
        try {
            ...
            if (data.usage) {
                ...
                if (modelInfo) {
                    this._stats.costUsd += (
                        (data.usage.prompt_tokens / 1_000_000) * modelInfo.costPerMInputTokens +
                        (data.usage.completion_tokens / 1_000_000) * modelInfo.costPerMOutputTokens
                    );
                }
            }
            ...
            this._stats.errors++;
            logger.error('Azure OpenAI request failed', { deployment: deploymentName, error: err.message });
            throw err;
        }
    }
    ...
    async smartRoute(messages, complexity = 'low', opts = {}) {
        const deploymentMap = {
            low: 'gpt-5-nano',        // $0.05/$0.40 — classification, routing
            medium: 'gpt-4.1-mini',   // $0.40/$1.60 — mid-complexity
