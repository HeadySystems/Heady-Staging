...
class LiquidACI extends EventEmitter {
  constructor(config = {}) {
    ...
    logger.info({ workspace: this.workspaceRoot }, 'LiquidACI initialized');
  }
  ...
  view(filePath, options = {}) {
    ...
    this._viewportState.set(absPath, { offset: end, totalLines });
    this._metrics.viewOps++;

    return {
      path: filePath,
      viewport,
      range: { start: offset + 1, end, total: totalLines },
  ...
  scrollDown(filePath) {
    return this.view(filePath, { offset: this._getViewportOffset(this._resolve(filePath)) });
  }

  scrollUp(filePath) {
    const absPath = this._resolve(filePath);
    const current = this._viewportState.get(absPath) || { offset: 0 };
    const newOffset = Math.max(0, current.offset - VIEWPORT_LINES * 2);
    return this.view(filePath, { offset: newOffset });
  }

  goToLine(filePath, lineNum) {
    const centered = Math.max(0, lineNum - Math.floor(VIEWPORT_LINES / 2));
    return this.view(filePath, { offset: centered });
  }
  ...
  edit(filePath, oldText, newText) {
    ...
    if (newText.split('\n').length > MAX_EDIT_LINES) {
      return { success: false, reason: `HEADY-ACI-001: Edit exceeds ${MAX_EDIT_LINES} line limit` };
    }
    ...
    if (occurrences === 0) {
      return { success: false, reason: 'HEADY-ACI-002: old_text not found in file' };
    }
    if (occurrences > 1) {
      return { success: false, reason: `HEADY-ACI-003: old_text matches ${occurrences} locations — provide more context` };
    }
    ...
    if (!lintResult.valid) {
      ...
      this.emit('edit:rejected', { filePath, reason: lintResult.error });
      return {
        success: false,
    ...
    this.emit('edit:applied', { filePath, linesChanged: newText.split('\n').length });

    logger.info({ filePath, linesChanged: newText.split('\n').length }, 'Edit applied');
  ...
  create(filePath, content) {
    ...
    if (fs.existsSync(absPath)) {
      return { success: false, reason: 'HEADY-ACI-005: File already exists — use edit()' };
    }
    ...
    if (!lintResult.valid) {
      return { success: false, reason: `HEADY-ACI-006: Content has syntax errors — ${lintResult.error}` };
    }

    fs.writeFileSync(absPath, content, 'utf-8');
    this.emit('file:created', { filePath });
    return { success: true, lines: content.split('\n').length };
  }
  ...
  undo(filePath) {
    ...
    if (!stack || stack.length === 0) {
      return { success: false, reason: 'HEADY-ACI-007: Nothing to undo' };
    }

    const prev = stack.pop();
    fs.writeFileSync(absPath, prev.content, 'utf-8');
    this._metrics.undoOps++;
    this.emit('edit:undone', { filePath });

    return { success: true, restoredTo: new Date(prev.timestamp).toISOString() };
  }
  ...
  search(pattern, options = {}) {
    ...
    try {
      ...
      const raw = execSync(cmd, { encoding: 'utf-8', timeout: fib(8) * 1000 });
      ...
      return {
        ...
        summary: results.length > 0
          ? `Found ${results.length} match${results.length > 1 ? 'es' : ''} across ${new Set(results.map(r => r.path)).size} file(s)`
          : 'No matches found',
      };
    } catch {
      return { pattern, matches: 0, results: [], summary: 'No matches found' };
    }
  }
  ...
  _lint(filePath, content) {
    ...
    if (!linterCmd) return { valid: true }; // no linter available, pass through
    ...
    try {
      ...
      execSync(cmd, { encoding: 'utf-8', timeout: fib(7) * 1000, stdio: 'pipe' });
      return { valid: true };
    } catch (e) {
  ...
  _pushUndo(absPath, content) {
    ...
    const stack = this._undoStack.get(absPath);
    stack.push({ content, timestamp: Date.now() });
...
module.exports = { LiquidACI, LINTER_COMMANDS };
