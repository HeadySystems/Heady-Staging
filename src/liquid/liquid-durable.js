/**
 * Heady™ LiquidDurable v1.0
 * Temporal-style durable execution with crash-proof workflow replay
 * Absorbed from: Temporal.io patterns (used by OpenAI Codex, Netflix)
 *
 * Workflows (deterministic orchestration) are separated from
 * Activities (non-deterministic side effects). If a process crashes,
 * state is transparently reconstructed from Event History replay.
 * Activities run once — results are recorded and replayed on recovery.
 *
...
class LiquidDurable extends EventEmitter {
  constructor(config = {}) {
    ...
    logger.info({ checkpointDir: this._checkpointDir }, 'LiquidDurable initialized');
  }

  // ── Register Activities ────────────────────────────────────────
  registerActivity(name, fn) {
    this._activityRegistry.set(name, fn);
    logger.debug({ activity: name }, 'Activity registered');
  }
  ...
  async startWorkflow(workflowId, workflowFn, input = {}) {
    ...
    if (checkpoint) {
      logger.info({ workflowId }, 'Resuming from checkpoint');
    ...
    execution.appendEvent(new DurableEvent(
    ...
    execution._heartbeatTimer = setInterval(() => {
      ...
      execution.appendEvent(new DurableEvent(
        EVENT_TYPES.ACTIVITY_HEARTBEAT,
        { timestamp: Date.now() },
        workflowId
      ));
    ...
    try {
      ...
      this.emit('workflow:failed', { workflowId, error: e.message });
      throw e;
    ...
    return {
      ...
      async executeActivity(name, input = {}, options = {}) {
        ...
        if (existingResult !== undefined) {
          logger.debug({ activityId, name }, 'Replaying activity result');
          return existingResult;
        }

        execution.appendEvent(new DurableEvent(
          EVENT_TYPES.ACTIVITY_SCHEDULED,
          { activityId, name, input },
          workflowId
        ));
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
          try {
            execution.appendEvent(new DurableEvent(
              EVENT_TYPES.ACTIVITY_STARTED,
              { activityId, name, attempt },
              workflowId
            ));
            ...
            const result = await Promise.race([
              ...
              new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Activity timeout')), timeout)
              ),
            ]);

            execution.appendEvent(new DurableEvent(
              EVENT_TYPES.ACTIVITY_COMPLETED,
              { activityId, name, result },
              workflowId
            ));
            ...
            lastError = e;
            execution.appendEvent(new DurableEvent(
              EVENT_TYPES.ACTIVITY_FAILED,
              { activityId, name, error: e.message, attempt },
              workflowId
            ));
        ...
        return new Promise((resolve, reject) => {
          const handler = (signal) => {
            if (signal.name === signalName && signal.workflowId === workflowId) {
              execution.appendEvent(new DurableEvent(
                EVENT_TYPES.SIGNAL_RECEIVED,
                { signalName, data: signal.data },
                workflowId
              ));
      ...
      get eventCount() { return execution.eventHistory.length; },
    };
  }

  // ── Signals ────────────────────────────────────────────────────
  sendSignal(workflowId, signalName, data = {}) {
    this.emit('signal', { workflowId, name: signalName, data, timestamp: Date.now() });
  }

  // ── Replay ─────────────────────────────────────────────────────
  async _replayWorkflow(workflowId, workflowFn, checkpoint) {
    this._metrics.replaysPerformed++;
    logger.info({ workflowId, events: checkpoint.eventHistory.length }, 'Replaying workflow');
  ...
  _findActivityResult(workflowId, activityName, input) {
    ...
    for (const event of execution.eventHistory) {
      if (event.type === EVENT_TYPES.ACTIVITY_COMPLETED &&
          event.payload.name === activityName) {
  ...
  _saveCheckpoint(workflowId) {
    ...
    try {
      ...
      logger.error({ workflowId, error: e.message }, 'Checkpoint save failed');
    }
  }
...
module.exports = { LiquidDurable, WORKFLOW_STATES, EVENT_TYPES };
