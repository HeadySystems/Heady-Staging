 ...
 * ═════════════════════════
 * Central async message broker for inter-service communication.
 * Domain subjects: heady.memory.*, heady.inference.*, heady.agents.*
 * Durable delivery with dead letter queues.
 *
 * φ-derived:
 *   - Retry policy: φ³ ≈ 4 attempts
 *   - Backoff: φ-exponential (1s, 1.618s, 2.618s, 4.236s)
 *   - Ack wait: 21s (Fibonacci)
 *   - Max pending: 34 (Fibonacci)
 */
...
class NATSJetStreamBus {
    ...
    constructor(opts = {}) {
        ...
        this._metrics = { published: 0, consumed: 0, dlq: 0, errors: 0 };
    }
        ...
        try {
            ...
            if (!this._nc) {
                this._nc = await nats.connect({
                    servers: this._url,
                    reconnect: true,
                    maxReconnectAttempts: FIB[10], // 89
            ...
            try {
                ...
                await jsm.streams.add({
                    ...
                    subjects: ['heady.>'],
                    retention: 'limits',
                    max_msgs: FIB[15] * 1000, // 987,000
                    max_bytes: FIB[13] * 1024 * 1024, // 377 MB
                    max_age: FIB[10] * 3600 * 1e9, // 89 hours in nanoseconds
                    storage: 'file',
                    num_replicas: 1,
                    discard: 'old',
                });
            ...
            _log('info', 'Connected to NATS JetStream', { url: this._url, stream: this._streamName });
        } catch (err) {
            _log('error', 'Failed to connect to NATS', { error: err.message, url: this._url });
            throw err;
        }
    }
    ...
    async publish(subject, data, headers = {}) {
        ...
        const ack = await this._js.publish(resolvedSubject, payload, { headers: msgHeaders });
    ...
    /**
     * Subscribe to a domain subject with durable consumer.
     *
     * @param {string} subject       - Subject key or raw subject
     * @param {string} consumerName  - Durable consumer name
     * @param {Function} handler     - async (msg) => void
     * @param {object} [opts]
     * @param {number} [opts.maxRetries]   - Max delivery attempts (default: φ³ ≈ 4)
     * @param {number} [opts.ackWaitMs]    - Ack timeout (default: 21s)
     * @param {number} [opts.maxPending]   - Max pending messages (default: 34)
     * @param {string} [opts.dlqSubject]   - Dead letter queue subject
     */
    async subscribe(subject, consumerName, handler, opts = {}) {
        ...
        try {
            ...
            await jsm.consumers.add(this._streamName, {
                ...
                ack_policy: 'explicit',
                ack_wait: ackWaitMs * 1e6, // Convert to nanoseconds
        ...
        const consume = async () => {
            while (this._connected) {
                try {
                    const fetched = await sub.fetch({ batch: FIB[5], expires: FIB[6] * 1000 }); // 8 msgs, 13s timeout
        ...
        consume().catch(err => _log('error', 'Consume loop exited', { error: err.message }));
        this._subscribers.set(consumerName, sub);

        _log('info', 'Subscribed', { subject: resolvedSubject, consumer: consumerName, maxRetries, ackWaitMs });
    }
    ...
    async _sendToDLQ(dlqSubject, originalMsg, error) {
        try {
            ...
            _log('error', 'Failed to send to DLQ', { error: dlqErr.message });
        }
    }
...
function _log(level, msg, meta = {}) {
    process.stdout.write(JSON.stringify({ level, service: 'nats-jetstream-bus', message: msg, ...meta, timestamp: new Date().toISOString() }) + '\n');
}

async function _importNATS() {
    try { return await import('nats'); } catch {
        try { return require('nats'); } catch {
            return { connect: () => { throw new Error('nats package not installed — run: npm install nats'); }, headers: () => new Map() };
        }
    }
}
