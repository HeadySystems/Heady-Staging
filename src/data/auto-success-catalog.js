...
try { extraTasks = require('../auto-flow-tasks.json'); } catch (e) {
  logger.warn(`[auto-success-catalog] Failed to load auto-flow-tasks.json: ${e.message}`);
}
let nonprofitTasks = [];
try { nonprofitTasks = require('../nonprofit-tasks.json'); } catch (e) {
  logger.warn(`[auto-success-catalog] Failed to load nonprofit-tasks.json: ${e.message}`);
}
let buddyTasks = [];
try { buddyTasks = require('../buddy-tasks.json'); } catch (e) {
  logger.warn(`[auto-success-catalog] Failed to load buddy-tasks.json: ${e.message}`);
}
let prodOptTasks = [];
try { prodOptTasks = require('./production-optimization-tasks.json'); } catch (e) {
  logger.warn(`[auto-success-catalog] Failed to load production-optimization-tasks.json: ${e.message}`);
}
...
try {
  ...
  for (const file of JSON_TASK_FILES) {
    ...
    try {
      ...
      logger.warn(`[auto-success-catalog] Failed to load JSON task file "${file}": ${e.message}`);
    }
  }
