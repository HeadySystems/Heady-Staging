...
router.get('/services', async (req, res) => {
    const checks = {};
    try {
        const pulse = await fetch(`${process.env.HEADY_MANAGER_URL || 'https://0.0.0.0:3301'}/api/pulse`, { signal: AbortSignal.timeout(2000) });
        checks.manager = pulse.ok ? 'connected' : 'degraded';
    } catch { checks.manager = 'disconnected'; }

    try {
        const headylocal = await fetch(process.env.OLLAMA_BASE_URL ? `${process.env.OLLAMA_BASE_URL}/` : 'http://0.0.0.0:11434/', { signal: AbortSignal.timeout(2000) });
