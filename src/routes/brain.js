...
async function chatViaOllama(message, system, temperature, max_tokens, history) {
    ...
    return new Promise((resolve, reject) => {
        const req = http.request({
            hostname: process.env.HEADY_LOCAL_HOST || process.env.REMOTE_HOST || '0.0.0.0', port: parseInt(process.env.OLLAMA_PORT || "11434"), path: "/api/generate", method: "POST",
...
router.post("/embed", async (req, res) => {
    ...
    try {
        ...
        const result = await new Promise((resolve, reject) => {
            const req2 = http.request(
                {
                    hostname: process.env.HEADY_LOCAL_HOST || process.env.REMOTE_HOST || '0.0.0.0', port: parseInt(process.env.OLLAMA_PORT || "11434"), path: "/api/embeddings", method: "POST",
                    headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(payload) },
                    timeout: 15000
                },
                (res2) => {
    ...
    } catch (err) { // Fallback: hash-based pseudo-embedding for structural matching
