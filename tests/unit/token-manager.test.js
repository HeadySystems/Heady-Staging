...
runTest('generateToken produces valid JWT-like structure', () => {
  const token = generateToken({ uid: 'user_1' }, SECRET);
  const parts = token.split('.');
  assert.strictEqual(parts.length, 3, 'should have 3 parts');
});

runTest('generateToken includes payload fields', () => {
...
runTest('generateToken SHORT_SESSION_MS uses PHI_TIMING.PHI_7', () => {
  assert.strictEqual(SHORT_SESSION_MS, PHI_TIMING.PHI_7, 'short session should match PHI_7');
});

// ─── Token Verification Tests ───────────────────────────────────────────────

runTest('verifyToken succeeds for valid token', () => {
...
runTest('verifyToken fails for wrong secret', () => {
...
runTest('verifyToken fails for tampered payload', () => {
...
runTest('verifyToken fails for invalid structure', () => {
...
runTest('revokeToken prevents subsequent verification', () => {
...
runTest('refreshToken produces new valid token', () => {
...
runTest('refreshToken revokes old token', () => {
...
runTest('refreshToken fails for invalid token', () => {
...
runTest('buildCookieOptions returns httpOnly, secure, sameSite', () => {
...
runTest('buildCookieOptions rememberMe increases maxAge', () => {
