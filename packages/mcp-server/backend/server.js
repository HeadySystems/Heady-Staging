...
const VALID_API_KEYS = new Set([
  process.env.HEADY_MASTER_KEY,
  process.env.HEADY_INTERNAL_KEY,
...
function loadProprietaryModules() {
  ...
  } catch (e) { cslEngine = null; }

  try {
    // φ-math — Golden ratio calculations
    phiMath = require(path.join(HEADY_ROOT, 'packages', 'phi-math'));
  } catch (e) {
    phiMath = { PHI, scale: (v) => v * PHI, fibonacci: (n) => {
      const fib = [0, 1]; for (let i = 2; i <= n; i++) fib[i] = fib[i-1] + fib[i-2]; return fib;
    }};
  }
...
async function executeTool(name, args) {
  switch (name) {
    ...
    case 'heady_search': {
      ...
      return { results: [], message: 'Vector memory module loading' };
    }
    ...
    case 'heady_store': {
      ...
      return { stored: false, message: 'Vector memory module loading' };
    }
    ...
    case 'heady_bee_list': {
      ...
      return { bees: [], swarms: [], message: 'Orchestrator loading' };
    }
    ...
    case 'heady_cost_report': {
      if (governance && governance.handleTool) {
        return governance.handleTool('heady_cost_breakdown', { period: args.period || 'month' });
      }
      return { period: args.period || 'month', totalCost: 0, breakdown: [], message: 'Governance loading' };
    }

    case 'heady_audit_log': {
      if (governance && governance.handleTool) {
        return governance.handleTool('heady_audit_query', { action: args.action, limit: args.limit || 20 });
      }
      return { entries: [], message: 'Governance loading' };
    }
    ...
    case 'heady_ecosystem_map': {
      try {
        ...
        return { error: 'Ecosystem analyzer not available' };
      }
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/mcp/tools/call') {
    ...
    req.on('end', async () => {
      try {
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ result, executedAt: new Date().toISOString() }));
      } catch (err) {
  ...
  if (req.method === 'GET' && req.url === '/mcp/tools/list') {
    ...
    return res.end(JSON.stringify({
      tools: [
        'heady_search', 'heady_store', 'heady_health', 'heady_pipeline_run',
        'heady_analyze', 'heady_bee_list', 'heady_battle', 'heady_autocontext',
        'heady_swarm_dispatch', 'heady_cost_report', 'heady_audit_log',
        'heady_ecosystem_map', 'heady_ecosystem_dependencies'
      ],
      count: 13
    }));
  }
  ...
  res.end(JSON.stringify({ error: 'Not found', endpoints: ['/health', '/mcp/tools/call', '/mcp/tools/list'] }));
});

server.listen(PORT, () => {
  console.log(`[heady-mcp-backend] ∞ Private backend listening on :${PORT}`);
  console.log(`[heady-mcp-backend] Auth keys configured: ${VALID_API_KEYS.size}`);
  console.log(`[heady-mcp-backend] Modules: CSL=${!!cslEngine} φ-math=${!!phiMath} VecMem=${!!vectorMemory} Orch=${!!orchestrator} Intel=${!!intelligence} Gov=${!!governance}`);
});
