...
function loadStore() {
  ...
  catch { return { replays: [], conversations: [], version: 1 }; }
}
...
function createReplay(conversationId, model, temperature, systemPrompt) {
  return {
    id: `replay_${Date.now()}`, conversationId, model, temperature: temperature || 0.7,
    systemPrompt: systemPrompt || 'default',
    created: new Date().toISOString(), divergences: [], status: 'pending'
  };
}
...
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/health') return res.end(JSON.stringify({ status: 'ok', service: 'time-machine' }));
  if (parsed.pathname === '/replay' && req.method === 'POST') {
    let body = '';
    req.on('data', c => body += c);
    req.on('end', () => {
      const { conversationId, model, temperature, systemPrompt } = JSON.parse(body);
      const store = loadStore();
      const replay = createReplay(conversationId, model, temperature, systemPrompt);
      store.replays.push(replay);
      store.version++;
      saveStore(store);
      res.end(JSON.stringify({ created: replay }));
    });
    return;
  }
  if (parsed.pathname === '/replays') return res.end(JSON.stringify(loadStore()));
  res.end(JSON.stringify({ service: 'Time Machine', version: '1.0.0',
    endpoints: { '/replay': 'POST', '/replays': 'GET' } }));
});

const PORT = process.env.PORT || 8106;
server.listen(PORT, () => console.log(`⏰ Time Machine on :${PORT}`));
