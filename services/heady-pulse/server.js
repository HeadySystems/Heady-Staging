...
function discoverServices(){
  ...
  try{
    fs.readdirSync(SERVICES_DIR).filter(d=>{
      ...
      services.push({name:d,port:portMatch?parseInt(portMatch[1]):null});
    });
  }catch{}
  return services;
}
async function checkHealth(serviceUrl){
  return new Promise(resolve=>{
    ...
    const req=proto.get(serviceUrl,{timeout:5000},r=>{
      let body='';r.on('data',c=>body+=c);
      r.on('end',()=>resolve({status:r.statusCode,latency:Date.now()-start,healthy:r.statusCode===200}));
    });
    req.on('error',e=>resolve({status:0,latency:Date.now()-start,error:e.message,healthy:false}));
    req.on('timeout',()=>{req.destroy();resolve({status:0,latency:5000,error:'timeout',healthy:false})});
  });
}
const server=http.createServer(async(req,res)=>{
  ...
  if(parsed.pathname==='/pulse'){
    ...
    return res.end(JSON.stringify({services:results,healthy,total:results.length,healthRate:`${(healthy/Math.max(1,results.length)*100).toFixed(0)}%`,timestamp:new Date().toISOString()},null,2));
  }
  res.end(JSON.stringify({service:'HeadyPulse',version:'1.0.0',endpoints:{'/discover':'GET','/pulse':'GET'}}))
});
const PORT=process.env.PORT||8132;server.listen(PORT,()=>console.log(`💓 HeadyPulse on :${PORT}`));
module.exports={discoverServices,checkHealth};
