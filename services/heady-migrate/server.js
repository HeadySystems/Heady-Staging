...
function loadStore() {
  ...
  catch { return { migrations: [], version: 1 }; }
}
...
function simulateMigration(from, to, changes) {
  const riskFactors = changes.map(c => {
    ...
    return { ...c, risk: Math.min(1, risk), rollbackable: risk < 0.8 };
  });
  const overallRisk = riskFactors.reduce((s, r) => s + r.risk, 0) / riskFactors.length;
  return {
    id: `mig_${Date.now()}`, from, to, changes: riskFactors,
...
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/health') return res.end(JSON.stringify({ status: 'ok', service: 'heady-migrate' }));
  if (parsed.pathname === '/simulate' && req.method === 'POST') {
    let body = '';
    req.on('data', c => body += c);
    req.on('end', () => {
      const { from, to, changes } = JSON.parse(body);
  ...
  res.end(JSON.stringify({ service: 'HeadyMigrate', version: '1.0.0', endpoints: { '/simulate': 'POST', '/migrations': 'GET' } }));
});
const PORT = process.env.PORT || 8109;
server.listen(PORT, () => console.log(`🔄 HeadyMigrate on :${PORT}`));
