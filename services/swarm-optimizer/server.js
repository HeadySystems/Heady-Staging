...
function loadStore(){try{return JSON.parse(fs.readFileSync(STORE_PATH,'utf8'))}catch{return{configs:[],benchmarks:[],version:1}}}
...
function optimizeConfig(taskType,metrics){
  const weights=SWARMS.map((s,i)=>({swarm:s,weight:Math.max(0.1,1-Math.pow(1/PHI,metrics?.[s]||1))}));
  const sorted=weights.sort((a,b)=>b.weight-a.weight);
  return{taskType,primarySwarms:sorted.slice(0,5).map(s=>s.swarm),weights:sorted,phiBalanced:true,optimizedAt:new Date().toISOString()};
}
const server=http.createServer((req,res)=>{
  ...
  res.end(JSON.stringify({service:'Swarm Optimizer',version:'1.0.0',endpoints:{'/optimize':'POST','/swarms':'GET'}}))
});
const PORT=process.env.PORT||8123;server.listen(PORT,()=>console.log(`🐝 Swarm Optimizer on :${PORT}`));
module.exports={optimizeConfig,SWARMS};
