...
function goldenSpiralLayout(elements, canvasWidth = 1200, canvasHeight = 800) {
  ...
  return elements.map((el, i) => {
    ...
    return {
      ...el, index: i, x: Math.round(cx + r * Math.cos(angle)),
      y: Math.round(cy + r * Math.sin(angle)),
      scale: parseFloat(scale.toFixed(3)),
      priority: elements.length - i,
      width: Math.round(200 * scale), height: Math.round(120 * scale),
...
function fibonacciGrid(items, columns = 5) {
  ...
  return items.map((item, i) => {
    ...
    return { ...item, col, row, relativeWidth: fibWidth, gridArea: `${row + 1} / ${col + 1}` };
  });
}
...
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/health') return res.end(JSON.stringify({ status: 'ok', service: 'golden-spiral-ui' }));

  if (parsed.pathname === '/layout' && req.method === 'POST') {
    let body = '';
    req.on('data', c => body += c);
    req.on('end', () => {
      const { elements, width, height } = JSON.parse(body);
  ...
  if (parsed.pathname === '/grid' && req.method === 'POST') {
    ...
    req.on('data', c => body += c);
    req.on('end', () => {
      const { items, columns } = JSON.parse(body);
      res.end(JSON.stringify(fibonacciGrid(items || [], columns)));
    });
    return;
  }
  if (parsed.pathname === '/demo') {
    const demo = goldenSpiralLayout(
      Array.from({ length: 20 }, (_, i) => ({ id: i + 1, label: `Element ${i + 1}`, content: `Priority ${20 - i} item` }))
    );
    return res.end(JSON.stringify(demo, null, 2));
  }
  res.end(JSON.stringify({ service: 'Golden Spiral UI', version: '1.0.0',
    endpoints: { '/layout': 'POST', '/grid': 'POST', '/demo': 'GET' } }));
});

const PORT = process.env.PORT || 8107;
server.listen(PORT, () => console.log(`🌀 Golden Spiral UI on :${PORT}`));
module.exports = { goldenSpiralLayout, fibonacciGrid };
