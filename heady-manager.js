...
// ─── Sentry Request Handler (must be early — before routes) ─────────
try {
...
// ─── Upstash Redis (HTTP REST — Cloud Run Optimized) ────────────────
try {
  const upstash = require("./src/services/upstash-redis");
  upstash.redisRoutes(app);

  // Auto-connect Upstash client to the Redis Sync Bridge for spatial cache
  try {
    const { RedisSyncBridge, registerRoutes: registerBridgeRoutes } = require("./src/services/redis-sync-bridge");
    const bridge = new RedisSyncBridge();
    if (upstash.isConfigured) {
      bridge.connectRedis(upstash.getRedisClient());
    }
    registerBridgeRoutes(app, bridge);
    logger.logNodeActivity("CONDUCTOR", `  🔴 Redis Sync Bridge: LOADED (mode: ${bridge.mode}) → /api/redis-bridge/*`);
  } catch (bridgeErr) {
    logger.logNodeActivity("CONDUCTOR", `  ⚠ Redis Sync Bridge not loaded: ${bridgeErr.message}`);
  }

  const mode = upstash.isConfigured ? 'UPSTASH REST' : 'IN-MEMORY FALLBACK';
  logger.logNodeActivity("CONDUCTOR", `  🔴 Upstash Redis: ${mode} → /api/redis/* (health, get, set, del)`);
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Upstash Redis not loaded: ${err.message}`);
}

// ─── Liquid State Lifecycle Manager (Gemini Liquid Architecture) ──────
try {
  const liquidState = require("./src/services/liquid-state-manager");
  liquidState.boot();
  liquidState.liquidStateRoutes(app);
  logger.logNodeActivity("CONDUCTOR", `  🌊 Liquid State Manager: BOOTED (${Object.keys(liquidState.getLifecycleMap()).length} targets) → /api/liquid-state/*`);
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Liquid State Manager not loaded: ${err.message}`);
}

// ─── IDE Bridge (HeadyAI-IDE Code Modification) ──────────────────────
try {
  const { ideBridgeRoutes } = require("./src/services/ide-bridge");
  ideBridgeRoutes(app);
...
// ─── Projection Engine + Governance + Domain Router ──────────────────
try {
  const { projectionRoutes } = require("./src/services/projection-engine");
  projectionRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  📡 Projection Engine: LOADED → /api/projections/*");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Projection Engine not loaded: ${err.message}`);
}

try {
  const { governanceRoutes } = require("./src/services/projection-governance");
  governanceRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  ⚖️  Projection Governance: LOADED → /api/governance/*");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Projection Governance not loaded: ${err.message}`);
}

try {
  const { domainRouterRoutes } = require("./src/services/domain-router");
  domainRouterRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  🌍 Domain Router: LOADED → /api/domains/*");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Domain Router not loaded: ${err.message}`);
}

// ─── UI Registry (HeadyWeb Universal Shell Manifest) ─────────────────
try {
  const { uiRegistryRoutes } = require("./src/services/ui-registry");
  uiRegistryRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  🎨 UI Registry: LOADED → /api/ui/* (manifest, health, register)");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ UI Registry not loaded: ${err.message}`);
}

// ─── LLM Router (Multi-Model Task Routing) ───────────────────────────
try {
  const { llmRouterRoutes } = require("./src/services/llm-router");
  llmRouterRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  🧠 LLM Router: LOADED → /api/llm/* (route, failover, health)");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ LLM Router not loaded: ${err.message}`);
}

// ─── Autonomous Scheduler (Cron-Based Bee Maintenance) ───────────────
try {
  const scheduler = require("./src/services/autonomous-scheduler");
  scheduler.start();
  scheduler.schedulerRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  ⏰ Autonomous Scheduler: STARTED → /api/scheduler/* (5 built-in schedules)");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Autonomous Scheduler not started: ${err.message}`);
}

// ─── Swarm Ignition (Bee Swarm Boot + Lifecycle) ─────────────────────
try {
  const { igniteSwarm, swarmIgnitionRoutes } = require("./src/orchestration/swarm-ignition");
  igniteSwarm({ enablePruner: true, enableTester: true, enableEmbedder: true });
  swarmIgnitionRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  🐝 Swarm Ignition: IGNITED → /api/swarm/* (launch, status, shutdown)");
} catch (err) {
...
try {
  ...
  logger.logNodeActivity("CONDUCTOR", "  📋 Bee Template Registry: LOADED → /api/templates/* (list, recommend, build-plan)");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Bee Template Registry not loaded: ${err.message}`);
}

// ─── Swarm Consensus (Distributed File Locking) ─────────────────────
try {
  const { consensus, registerConsensusRoutes } = require("./src/orchestration/swarm-consensus");
  consensus.startStaleCheck();
  registerConsensusRoutes(app);
  logger.logNodeActivity("CONDUCTOR", "  🔒 Swarm Consensus: LOADED → /api/consensus/* (locks, heartbeat)");
} catch (err) {
  logger.logNodeActivity("CONDUCTOR", `  ⚠ Swarm Consensus not loaded: ${err.message}`);
}
...
// ─── Sentry Error Handler (captures exceptions before response) ─────
try {
  const sentry = require("./src/services/sentry");
...
app.use((req, res) => {
  res.status(404).json({
    ...
    path: req.originalUrl,
    method: req.method,
    hint: 'Try GET /api/health or GET /api-docs for available endpoints',
    timestamp: new Date().toISOString(),
  });
});
